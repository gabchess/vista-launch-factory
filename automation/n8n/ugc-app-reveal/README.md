...
The input needs `campaign_id`, `request_id`, `recipe_id`, `deliverable_scope`, `product`, `format`, `brief` and `bindings`. `standalone_product_film` allows up to 60 seconds. `vista_social_video` retains the brief's 30-second cap. Eight required content bindings come from `recipe.json`. Stage-specific inputs, such as dialogue timestamps or a listing manifest, are resolved before that prompt runs.
