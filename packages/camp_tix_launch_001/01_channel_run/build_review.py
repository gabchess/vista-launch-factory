...
def main():
    ...
    assets.append(row('blog','Blog post',b['title'],'Feature article · '+str(b['estimated_reading_minutes'])+' minute read','iframe','blog/article.html','blog/article.md',['blog/article.md','blog/article.html','blog/metadata.json','blog/provenance.json','blog/assets/tix-hero.png','blog/assets/tix-camera-brief.png'],'blog',['TIX-C01','TIX-C02','TIX-C03','TIX-C04','TIX-C05','TIX-C06','TIX-C07','TIX-C08'],'Help a reader understand the conversation and form a useful buying brief.','Based on four externally observed feature-launch articles. Tix remains the product and publisher.','Article, SEO fields and two images are included. Confirm copy and the waitlist destination before publishing.','blog',1200,version=b['version']))
    email=load('email/emails.json')
    for v in email['variants']:
        name=v['id'];ids=sorted(set(c for p in v['paragraphs'] for c in p['claim_ids']))
        a=row('email-'+name,v['label'],v['subject'],v['preheader'],'iframe','email/'+name+'.html','email/'+name+'.md',['email/'+name+'.md','email/'+name+'.html','email/'+name+'.json'],'email_segments',ids,v['reader_job'],'An externally observed public feature register was adapted for email. No private newsletter corpus was supplied.','Audience labels are test contexts. Confirm the real list, waitlist collection and ESP footer before sending.','email',1000,version=v.get('version',email['version']))
        a['group']='Email variants';assets.append(a)
    c=load('changelog/entry.json')
    assets.append(row('changelog','Changelog',c['title'],'Concise product preview update','iframe','changelog/entry.html','changelog/entry.md',['changelog/entry.md','changelog/entry.html','changelog/entry.json','blog/assets/tix-camera-brief.png','popup/assets/Poppins-Regular.ttf','popup/assets/Poppins-SemiBold.ttf'],'changelog',['TIX-C01','TIX-C02','TIX-C03','TIX-C05','TIX-C06','TIX-C08'],'Give returning readers a short account of the preview and a first useful action.','Based on three externally observed changelog entries, including the most recent one.','Availability stays qualified as a preview. A public launch date has not been assigned.','changelog',1020,version=c['version']))
    p=load('popup/copy.json')
    assets.append(row('popup','In-app popup',p['copy']['headline'],'Graphic + copy · Desktop and mobile preview','iframe','popup/popup.html','popup/copy.json',['popup/popup.html','popup/graphic.svg','popup/copy.json'],'in_app_popup',['TIX-C01','TIX-C02','TIX-C08'],p['purpose'],"An externally observed login page's typography and published palette informed the original popup layout.",'The modal is an original design. Review the message and artwork together; production targeting still needs integration.','popup',700,version=p['version']))
