...
4. **X/Twitter timeline as a whole** — only individual indexed posts could be fetched (5 captured: corpus `12`–`16`). No full recent timeline, no engagement context beyond like/RT counts on two posts.
...
2. **3–5 more Barry emails** — especially any approval-queue series siblings — so the email register isn't n=1.
