...
- **Current state:** Scope B + recorded-factory HOW are Gabe-approved (2026-09-07). Working tree under `vista/work/` uses a **labelled fixture** until Reggie shares the real release folder. This SOP is a working draft; owner approval is required before distribution. v0.1.1 repairs reviewer R3 to R7 only; does not invent live automation or close Reggie/owner gaps.
...
- **Active blockers or uncertainty:** G1 to G10 Reggie access/judgment; G11 process-owner / distribution-approver seats; G12 durable post-Wed Run trigger. The tracked **Run launch** control is a **demo leave-behind** (not evidenced as a live clickable control). Local validators/package scripts are **crew-run / manual demo** capabilities, not a marketer terminal path and not claimed live automation.
- **Authority currently required:** Gabe/Reggie for access and ownership; Barry for Claims Lock / O1 spot-check / pack approve; Gabe for cut-order fall-back to thin C; the contingency adviser advises cut-order order only (D7).
...
| Internal agent stack (STACK) | Builder-side only, not marketer Run |
...
**Start only when** a tracked Campaign row exists (or fixture campaign `camp_demo_001` is in use) and the operator knows whether sources are **fixture** or **Reggie-real**.

## Scope and boundaries

**Included:**

- Scope **B**: outputs O1 to O6 + full one-release cadence binder (tier blast cells + UTM stubs).
...
- Builder maintenance of the internal agent stack (STACK) / marketplace bots → builder role, not marketer Run.
...
| Fixture vs real | `fixture_label` non-empty **or** explicitly cleared after Reggie swap | the tracker's `fixture_label` field; `FIXTURE_LABEL.md` | If unclear, treat as fixture and label every artifact. |
...
| Barry seat | Writer seat ≠ Barry seat; Barry surface known (default approval surface + Drive pack) | Campaign `barry.seat`, `barry.surface` | Pause at Claims Lock / pack queue until surface confirmed. |
| WIP gate | Zero other campaigns in `awaiting_barry` (WIP=1) | the tracker's `barry_wip` field / campaign statuses | Do not advance a second pack into `awaiting_barry`. |
...
| Run trigger surface | Operator knows whether **demo leave-behind**, **manual rehearsal**, or **evidenced live** trigger applies | Campaign comment / row note: `trigger_used` | If the tracker's Run button is not clickable / not demonstrated → use **manual rehearsal fallback** (step 3). Do **not** call path marketer-live until one demonstrated trigger + proof on row (G12). |
...
|---|---|---|---|---|
| **Marketer (Run operator)** | Uses the tracked Campaign row after trigger is available (or follows manual rehearsal); confirms folder/fixture; watches status; submits pack to Barry; confirms package after crew build; optional HubSpot draft | When to start Run / rehearsal; when to skip optional sandbox | Folder present; WIP=1; honesty doc attached; `trigger_used` noted | Does **not** open a terminal; cannot approve Claims Lock or pack; cannot publish; cannot invent a live Run button |
| **Writer / adapters (crew or tools)** | Draft O1 to O6 + cadence from ledger + voice pack | None on ship | Validation scripts pass/fail | Never Barry; never silent rewrite-as-approve |
...
| **Gabe** | Access chase; cut-order / thin-C yes; process sponsor until ops owner named | Fall back to thin C; reopen Scope B | N/A | Authority for contingency labels |
| **Reggie (COO / sponsor)** | Folder/perms, brand samples, HubSpot sandbox invite, feature pick, format rules, off-limits claims | Real-feature intake; off-limits claim list | N/A | Blanks = keep fixture label |
...
| **Contingency adviser** | Advises cut-order drop sequence when clock slips (D7 / E7) | Advises order only, does **not** approve thin-C fall-back; does **not** reopen Scope A/B/C | Cut-order note accuracy vs `decision.md` | Thin C / scope fall-back requires **Gabe yes** + “bonus partial” label; adviser's advice ≠ target scope |
| **Draft-review lead** | Leads adapter draft quality; owns E5 hold decision when validate hard-fails or >2 retries; this role is the evidenced crew drafting / yes-gate seat for O2 to O4 briefs (eggbot wire; questions-for-reggie draft) | Hold vs fix-and-retry on validation hard fail (E5); not ship gate | Validator errors + `hold_reason` on record | Never Barry; never silent rewrite-as-approve; cannot clear Claims Lock |
...
|---|---|---|---|---|
| Tracked Campaign row | Status, Barry queue; **Run launch** = demo leave-behind Button (template + leave-behind doc), **not evidenced** as a live clickable control in a production tracking database | Pilot SoT status (intended) | Internal | Status transitions; `barry_wip=1`; `trigger_used` note |
...
| Validators | `validate_ledger.py`, `validate_campaign.py`: **crew-run / manual demo** local scripts (marketer has no terminal path) | Local `work/scripts/` | N/A | Pass/fail logs; retry count ≤2; result surfaced on Campaign row |
| `build_package.py` | Frozen package + honesty: **crew-run / manual demo** until non-terminal path evidenced | Drive package path | N/A | MANIFEST with `auto_publish: false` |
| HubSpot sandbox | Optional draft emails last | Reggie sandbox only | Credentials, operator only | `draft_ids[]`; status `draft_only` |
| Customer.io | Crew internal only | Not Vista ESP | N/A | Do not appear in Vista demo path |
| Automation-trigger / Drive label | Leave-behind Run triggers (same brain pickup; not Wed hero) | Documented leave-behinds only | N/A | Handoff note; record `trigger_used` if used |
...
3. **Start Run or manual rehearsal.** The tracked **Run launch** control is the **demo leave-behind** (campaign-template Button + `notion-run-leavebehind.md`), **not evidenced** as a live clickable control. Do **not** call this marketer-live until a trigger has been demonstrated once and proof is on the Campaign row (G12).  
   - **If a demonstrated automation-trigger platform is available:** Marketer uses that control. Observable result: status moves toward `ingested` / checklist comment with `folder_id`, claim_ledger link, WIP check. Record `trigger_used` on the row.  
...
6. **Present ledger for Claims Lock.** Writer/crew places claim ledger on the Barry surface (the approval card + linked ledger). Status `claims_gate`. Every **allowed** claim shows evidence span (transcript or outline). Forbidden list present. No pricing/limits invention.
...
17. **Re-run named slot only.** Marketer/writer notes Barry’s named slot number(s) (1 to 6) or `cadence`. Regenerate **only** those artifacts from current ledger (do not reopen Claims Lock unless claims themselves changed). Bump artifact `version`. Re-validate affected slots. Resubmit pack with WIP=1. If claims text must change → return to Claims Lock (step 6) before adapters.
...
| D3 | O1 spot-check pass? | Barry | Pass → O2 to O6; Changes → re-run slot 1 | Spot-check note |
| D4 | Validation pass within ≤2 retries? | Crew runs validators; draft-review lead on hard fail (E5); marketer confirms row | Pass → awaiting_barry; Hard fail → E5 | Validator log |
...
| D7 | Cut-order live (<24h / blocking)? | Gabe (thin C); contingency adviser advises order | Label bonus partial; never quiet thin C | Cut-order note on package |
...
| E5 | Validate hard fail or >2 retries | Status `held`; hold artifact on record with reason | Draft-review lead; Barry notified | Before pack submit | Fix → validate → step 12 | Validator errors + hold_reason |
...
| C2 Spine + Barry HITL sequence | Human decision | DESIGN-LOCK §1 to §2; barry templates | Skipping Claims Lock → false claims | Locked |
| C3 No auto-publish; HubSpot draft-only last; CIO ≠ ESP | Human decision | design §4 to §9; hubspot checklist | Brand/trust breach | Locked |
...
| C5 Who clicks Run = marketing | Human decision | design §5 to §6; trigger leave-behind | Role drift to eng/builder | Confirm on owner approval |
| C6 Builder ≠ maintainer | Human decision | design handoff must-names | Orphaned procedure | Name maintainer at approval |
| C7 Run trigger = demo leave-behind (not evidenced live clickable) | Bounded assumption | trigger leave-behind; campaign-template Button; reconstruction (marketers not yet day-to-day) | Calling path marketer-live without proof | Manual rehearsal until demonstrated trigger; G12 durable pick post-Wed |
| C8 Barry surface = approval surface + Drive pack | Bounded assumption | templates; Reggie Q4 unanswered | Wrong queue | Reggie answer |
| G1 to G10 Reggie questionnaire | Unresolved fact | questions-for-reggie.md blank | Block real-feature swap | Gabe sends; Reggie replies |
...
| G12 Durable post-Wed Run trigger (automation-platform choice vs Drive) | Unresolved fact | design leave-behinds; no evidenced live clickable Run | Marketer may lack a real Run control | Gabe/Reggie/ops pick + demonstrate once; proof on row before “live” |
...
| 0.1.1 | 2026-09-07 | Repair pass per review-report R3 to R7: companion paths; reviewer + writer roles; crew-run validate/package + tracker demo leave-behind + manual rehearsal; Terms table; G12 label aligned to durable trigger. G1 to G11 unchanged unresolved | sop-builder repair; not owner-approved | 0.1.0 |
