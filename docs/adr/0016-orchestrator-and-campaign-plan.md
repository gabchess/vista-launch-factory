...
Launch Factory gets an **orchestrator seat**: one agent at the front door, Nova-shaped.
...
spine (ingest → Claims Lock → adapters → validate → Barry).
...
by Barry as part of the pack.

## Why

The brief's bonus "counts for a lot": a campaign, not six disconnected assets. And the
orchestrator is what makes the system feel like the thing Reggie described (a machine
...
- Barry approves the Campaign Plan inside the same pack-approve gate; WIP=1 unchanged.
