...
| Artifact | Verified file properties | Current meaning |
| --- | --- | --- |
| Tix animation, `tixmancer motion.mp4` | 10 seconds, 1080 x 1080, 30 fps, H.264/AAC | Approved creative; login surface fit, loop and reduced-motion treatment remain to check |
| Product film, `Dinner-with-Tix-app-reveal-v3.mp4` | 55-second video, 1920 x 1080, 30 fps, H.264/AAC | Approved standalone master; the complete media container measures about 55.08 seconds |

Animation SHA256: `f16b7cfda2d6ef8671fc2add7a404cf77be321dffec56503a1326b387bb7b76a`.

Film SHA256: `1e73f611ecaacaa9aabe1b3cfdc53f571888573ae876aebb65674ad2903604d2`.

Gabe approved these assets for Tix. That does not create Barry's approval for Vista or approve a future rendition. Keep the accepted performances, speech, app reveal and exact waitlist ending. Shorten or adapt only when the selected deliverable needs it. A new cut returns for its affected human review.

The film combines accepted ChatCut performances, actor-picture enhancement through Higgsfield/ByteDance, authored HyperFrames UI and ChatCut assembly. The app source adapted AI Chat Reveal. Preserve that provenance. Real listing photographs were verified for the film, but there was no actual Tix search, account login, wallet import or payment in the authored app scene. Current listing availability needs fresh checking on reuse.

## Reusable system saved

- Twelve specialist roles, reference banks, typed requests/results and project entry points are present over the historical structural engine.
...
Tixmancer is the internal test product. Prospector is the selected client demonstration source. Reggie's public writing and Vista-authored material can supply expression references; only the product's selected source can establish its features, limits and pricing. Retrieve actual examples before claiming a complete voice bank.

The original brief calls for one real feature release through all six outputs and a recorded complete run. Demonstrating a GitHub input alone does not prove folder ingest or the full brief. Follow the user's chosen demo source and record which input modes and outputs were exercised.

The latest user request calls for a testable app without an engineer in the loop. [ADR 0019](adr/0019-testable-app-and-next-milestone.md) supersedes the earlier demo-only availability assumption for planning. Hosting duration, test access and spending limits must be explicit at deployment. The prior USD 500 ceiling is a ceiling, not evidence of remaining balance. Check actual provider cost and account access before new paid work.
